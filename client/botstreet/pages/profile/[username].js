...
const Profile = () => {
  ...
  const { username } = router.query; // Capture the dynamic part of the URL
  const [userId, setUserId] = useState('');
  const [email, setEmail] = useState('');

  useEffect(() => {
    if (username) {
      console.log('Username from query string:', username); // You can now log the dynamic username from the URL
      const storedUserId = localStorage.getItem('userId');
      const storedEmail = localStorage.getItem('email');
      setEmail(storedEmail || 'Unknown');
      setUserId(storedUserId || 'Unknown');
    }
  }, [username]); // Re-run the effect whenever the username changes
